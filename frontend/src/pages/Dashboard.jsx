  ...
  const selectedProductStats = useMemo(() => {
    ...
    const stats = productSales.reduce((acc, sale) => {
      ...
      if (item) {
        ...
        acc.revenue += item.quantity * (item.price || 0);
        acc.profit += (item.quantity * (item.price || 0)) - (item.quantity * product.purchasePrice);
          ...
          {bestSellers.length === 0 ? (
            ...
            <ul className={styles.list}>
              {bestSellers.map((product, index) => (
                <li key={product._id || product.id} className={styles.listItem}>
                  <span className={styles.itemName}>
                    <span className={styles.rank}>#{index + 1}</span>
                    {product.name}
                  </span>
